<script>
  ...
  // El contenedor sobre el que haremos scroll con GSAP
  let scrollContainer;

  // Detectamos si estamos en la ruta raíz
  ...
  function crearPalabra() {
    ...
    gsap.to(palabraFrente, {
      ...
      onComplete: () => {
        ...
        gsap.to(palabraFrente, { opacity: 1, y: 0, duration: 0.3 });
      }
    });
  }

  // Función genérica para hacer scroll a una posición Y fija
  ...
  // Maneja click en la foto (va a "About" o vuelve a home)
  ...
  onMount(() => {
    ...
    // 4) manejar la clase "bright" según scroll interno
    let backgroundEl, handleScroll;
    if (isHome) {
      backgroundEl = scrollContainer;
      handleScroll = () => {
        const scrollY = backgroundEl.scrollTop || 0;
        const scrollHeight = backgroundEl.scrollHeight || 0;
        const clientHeight = backgroundEl.clientHeight || 0;
        const reachedBottom = scrollY + clientHeight >= scrollHeight - 800;
        isBright = (scrollY > 1400 && scrollY < 8100) || reachedBottom;
      };
...
<header id="header" class="header-section start" class:home-background={isHome} class:bright={isBright}>
  <div class="profile-info">
    ...
    <AnimatedTooltip label="Let's work together!">
      <div
        class="personal-photo"
        on:click={() => {
          if (isHome) {
            document.querySelector('.background')?.scrollTo({ top: 8800, behavior: 'smooth' });
          } else {
            goto('/');
          }
        }}
      ...
      <div class="navigation-buttons">
        ...
        {#if isMobile}
          ...
          {#if menuOpen}
            <div class="mobile-menu">
              <div class="icon-button menu work" on:click={() => { document.querySelector('.background')?.scrollTo({ top: 1200, behavior: 'smooth' }); }}>
                <span>Works</span>
              </div>
              <div class="icon-button menu About" on:click={() => { document.querySelector('.background')?.scrollTo({ top: 8800, behavior: 'smooth' }); }}>
                <span>About</span>
              </div>
              <div class="icon-button menu contact" on:click={() => { document.querySelector('.background')?.scrollTo({ top: 16800, behavior: 'smooth' }); }}>
                <span>Contact</span>
              </div>
            </div>
          {/if}
        ...
        {:else}
          ...
          <div
            ...
            on:click={() => { document.querySelector('.background')?.scrollTo({ top: 2800, behavior: 'smooth' }); }}
          >
            <span>Works</span>
