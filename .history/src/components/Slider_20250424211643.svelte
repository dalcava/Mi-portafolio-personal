<script>
    ...
    function getTranslateFromScroll(scrollY) {
        ...
        const effectiveY = scrollY - start;

        // Antes de SCROLL_START, mantener en 0
        if (effectiveY <= 0) return 0;

        const index = Math.floor(effectiveY / step);
        const relativeScroll = effectiveY % step;

        let baseTranslate = index * (swiper.width / swiper.params.slidesPerView);
        
        // Suavizamos solo el primer paso
        if (index === 0) {
            // Aplicamos un easing tipo cubic o exponencial (easeOut)
            const progress = relativeScroll / step;
            const easedProgress = 1 - Math.pow(1 - progress, 3); // easeOutCubic
            baseTranslate += easedProgress * (swiper.width / swiper.params.slidesPerView);
        } else {
            baseTranslate += relativeScroll * 0.3;
        }
    ...
    function handleScrollPositionSync() {
        ...
        // Si ya hay una animación corriendo, detenla
        if (currentTween) {
            currentTween.kill();
        }

        // Usa GSAP para mover el Swiper de forma suave
