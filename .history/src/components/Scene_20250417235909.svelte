...
loader.load(
  ...
  (gltf) => {
    ...
    const divineGlass = new THREE.ShaderMaterial({
  uniforms: {
    uTime: { value: 0 },
    uEnvMap: { value: yourHDRTexture }, // cubemap or equirect
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      vec3 color = mix(chroma, uTint, 0.1) + fresnel * vec3(1.0); // glow from edge
