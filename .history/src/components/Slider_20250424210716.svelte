<script>
    ...
    let categoryElement;
    let textElement;
    const AUTOPLAY_DELAY = 6400;
    const SCROLL_START = 2300;
    ...
    function handleScrollToSwiperSync() {
        if (!swiper || isProgrammaticScroll || !allowScrollNavigation) return;

        const scrollY = scrollContainer?.scrollTop ?? window.scrollY;

        // Mapeamos scrollY a translate de swiper
        const totalScrollableHeight = document.body.scrollHeight - window.innerHeight;
        const maxSwiperTranslate = swiper.maxTranslate(); // ya negativo
        const minSwiperTranslate = swiper.minTranslate(); // cero o positivo

        // Normalizamos la posición del scroll (entre 0 y 1)
        const progress = Math.min(Math.max(scrollY / totalScrollableHeight, 0), 1);

        const newTranslate = minSwiperTranslate + (maxSwiperTranslate - minSwiperTranslate) * progress;
    ...
    function handleScrollPositionSync() {
        if (isProgrammaticScroll || !allowScrollNavigation) return;
        ...
        if (index !== -1 && swiper && swiper.realIndex !== index) {
            // Esto evita un loop de animaciones
            if (!isAnimating) {
                swiper.slideTo(index, 600, false); // ⚠️ false = sin animación interna (para que se sienta scroll)
            }
        }
    }
    ...
    export function scrollToStart() {
        ...
        target.scrollTo({
            top: SCROLL_START,
            behavior: 'auto' // Podés poner 'smooth' si querés animarlo
        });

        setTimeout(() => {
            isProgrammaticScroll = false;
        }, 300); // Delay cortito para que no entre en conflicto con otras animaciones
    }
    ...
    onMount(() => {
        ...
        if (scrollContainer) {
            scrollContainer.addEventListener('scroll', handleScrollToSwiperSync);
        }
