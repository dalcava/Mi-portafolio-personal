...
const glassMaterial = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      // Slight desaturation to help tint shine
      float gray = dot(chroma, vec3(0.299, 0.587, 0.114));
      chroma = mix(vec3(gray), chroma, 0.6);

      // Tint + edge glow
      vec3 color = mix(chroma, uTint, 0.3) + fresnel * vec3(1.5);

      // Internal radiant core glow
      float glow = pow(dot(normalize(vWorldNormal), normalize(vWorldPosition - cameraPosition)), 3.0);
      color += uTint * glow * 0.5;

      // Brighten midtones
      color = pow(color, vec3(1.0 / 1.2));

      // Final alpha balance
      float alpha = 0.985 + fresnel * 0.015;
