...
const glassMaterial = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      vec3 chroma = vec3(refR.r, refG.g, refB.b) * 0.5; // soften base

      float coreGlow = pow(dot(normalize(vWorldNormal), normalize(vWorldPosition - cameraPosition)), 2.0);
      vec3 glowColor = uTint * coreGlow * 1.25;

      vec3 fresnelGlow = vec3(1.0, 0.8, 1.0) * fresnel * 1.2;

      vec3 color = mix(chroma, uTint, 0.2) + glowColor + fresnelGlow;

      // Gentle brightening
      color = pow(color, vec3(1.0 / 1.1));

      float alpha = 0.96 + fresnel * 0.03;
