    ...
    if (model) {
      ...
      if (scrollY >= entryPoint && scrollY <= exitPoint) {
        if (!model.visible) {
          ...
          // Asegura que no haya animaciones pendientes de posición u opacidad
          gsap.killTweensOf(model.position);
          gsap.killTweensOf(model.material);

          // Entrada suave desde la izquierda + opacidad
        ...
        // Fuera del rango, animar salida si está visible
        if (model.visible) {
          gsap.killTweensOf(model.position);
          gsap.killTweensOf(model.material);
          ...
          gsap.to(model.position, {
            x: -3,
            y: model.position.y - 0.5, // Baja un poco al salir
            duration: 2,
            ease: 'power4.inOut'
          });
