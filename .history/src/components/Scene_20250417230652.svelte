  ...
  (gltf) => {
    ...
    if (backgroundEl) {
      backgroundEl.addEventListener('scroll', () => {
        ...
        const triggerScroll = 5200;

        if (model) {
          if (scrollY >= triggerScroll) {
            const relativeScroll = scrollY - triggerScroll;

            const targetRotationY = 3 + relativeScroll * -0.0015;
            const targetRotationX = relativeScroll * 0.0005; // Factor más pequeño para inclinación
