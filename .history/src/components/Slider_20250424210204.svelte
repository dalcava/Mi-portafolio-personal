<script>
    ...
    function syncSwiperWithScrollSnapping() {
        ...
        if (index !== -1 && swiper) {
            // Scroll en tiempo real, sin animación
            swiper.slideTo(index, 0, false);

            // Cancelamos el snap anterior si el usuario sigue scrolleando
            clearTimeout(snapTimeout);
            if (!isSnapping) {
                snapTimeout = setTimeout(() => {
                    // Snapping con animación suave
                    swiper.slideTo(index, 800); // duración del efecto elástico
                    isSnapping = false;
                }, 160); // Ajustá este tiempo para detectar "fin del scroll"
            }
    ...
    onMount(() => {
        ...
        if (scrollContainer) {
            scrollContainer.addEventListener('scroll', syncSwiperWithScrollSnapping);
        }
        ...
        const cleanup = () => {
            if (scrollContainer) {
                scrollContainer.removeEventListener('scroll', syncSwiperWithScrollSnapping);
            }
        };
