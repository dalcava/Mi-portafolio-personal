...
const glassMaterial = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      vec3 color = mix(chroma, uTint, 0.1) + fresnel * vec3(1.0);

      float alpha = 0.93 + fresnel * 0.07; // <-- Aquí defines la opacidad dinámica
      gl_FragColor = vec4(color, alpha);     // <-- La aplicas al color final
    }
