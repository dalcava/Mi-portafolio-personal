...
const glassMaterial = new THREE.ShaderMaterial({
  uniforms: {
    ...
    uBaseColor: { value: new THREE.Color('#cfd8dc') }, // Moon gray
    uThreadColor: { value: new THREE.Color('#ffffff') },
  },
  vertexShader: `
    varying vec2 vUv;
  ...
  fragmentShader: `
    ...
    float pattern(vec2 uv) {
      vec2 grid = fract(uv * uNoiseScale);
    ...
    void main() {
      ...
      // Base suit texture (woven grid-like shimmer)
      float weave = pattern(vUv + vec2(sin(uTime), cos(uTime)) * 0.01);

      vec3 base = uBaseColor;

      // Iridescent tint shift
      vec3 iridescence = mix(vec3(1.0, 0.8, 0.9), vec3(0.6, 0.9, 1.0), sin(uTime + vUv.y * 20.0)) * uIridescenceStrength;

      // Simulate stitched threads catching light
      vec3 threadHighlight = uThreadColor * weave * uSpecularStrength;

      // Combine layers
      vec3 finalColor = base + threadHighlight + fresnel * iridescence;
