...
loader.load(
  ...
  backgroundEl.addEventListener('scroll', () => {
    ...
    if (model) {
      // Mostrar el modelo entre entryPoint y exitPoint
      if (scrollY >= entryPoint && scrollY <= exitPoint) {
        if (!model.visible) {
          model.visible = true;

          // Reset de animaciones anteriores
          gsap.killTweensOf(model.position);
          gsap.killTweensOf(model.material);

          // Restaurar posición y opacidad antes de entrar
          model.position.y = 1;
          model.material.opacity = 0;

          // Entrada animada desde la izquierda + opacidad + sutil subida
        ...
        // Si está fuera del rango y visible, sale animado
        if (model.visible) {
          gsap.killTweensOf(model.position);
          gsap.killTweensOf(model.material);

          // Salida animada hacia la izquierda con bajada y desvanecimiento
          gsap.to(model.position, {
            x: -3,
            y: model.position.y - 0.3,
            duration: 1.5,
            ease: 'power4.inOut'
          });
