...
const glassMaterial = new THREE.ShaderMaterial({
  uniforms: {
    ...
    uPanelFrequency: { value: 15.0 },
    uFresnelPower: { value: 3.0 },
    uColorA: { value: new THREE.Color('#cfd8dc') }, // Outer shell tone
    uColorB: { value: new THREE.Color('#262c3a') }, // Inner shadow tone
    uGlowColor: { value: new THREE.Color('#ffaaff') }
  },
  vertexShader: `
  ...
  fragmentShader: `
    ...
    float panelLines(vec2 uv) {
      vec2 grid = fract(uv * uPanelFrequency);
      float line = step(0.98, grid.x) + step(0.98, grid.y);
      return line;
    }

    vec3 iridescentMix(vec3 base, float fresnel, float time) {
      return mix(base, 
                 vec3(sin(time + fresnel * 5.0), 
                      sin(time * 0.5 + fresnel * 7.0), 
                      sin(time * 0.8 + fresnel * 3.0)) * 0.5 + 0.5,
                 fresnel);
    }
    ...
    void main() {
      ...
      float panels = panelLines(vUv + vec2(sin(uTime * 0.2), cos(uTime * 0.1)) * 0.1);
      vec3 panelEffect = mix(uColorA, uColorB, panels);

      vec3 iridescence = iridescentMix(panelEffect, fresnel, uTime);

      vec3 finalColor = mix(iridescence, uGlowColor, fresnel * 0.3);
...
const chromeMaterial = new THREE.MeshStandardMaterial({
  ...
  spaceHullMaterial.uniforms.uTime.value += clock.getDelta();
};
