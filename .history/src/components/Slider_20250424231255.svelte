<script>
    ...
    function manageScrollNavigation() {
        ...
        if (scrollY < scrollFreezeThreshold && allowScrollNavigation) {
            // ⛔ Desactiva sincronización con scroll cuando sube demasiado
    ...
    export function scrollToStart() {
        ...
        // Solo ejecuta si el scroll es menor a 8200
        if (currentScroll > 5200) {
            console.log("[scrollToStart] Ignorado porque scroll actual es mayor a 8200:", currentScroll);
            return;
        }

        console.log("[scrollToStart] Ejecutado: scroll actual =", currentScroll);
...
/* 
            if (swiper.isEnd) {
                ...
                return; // <-- Evita que ejecute más lógica abajo
            } */
            ...
            if (y) {
                // Si tenés lógica de scroll acá, asegurate de que no se ejecute si isEnd es true
                // Por ejemplo:
