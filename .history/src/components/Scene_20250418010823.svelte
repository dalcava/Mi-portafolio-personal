  ...
  (gltf) => {
    ...
    if (backgroundEl) {
      backgroundEl.addEventListener('scroll', () => {
        ...
        const entryPoint = 5600;
        const exitPoint = 7300;
    ...
    if (model && !isAnimating && isInsideRange) {
      ...
      const desiredRotationY = 3 + relativeScroll * 5;
      const desiredRotationX = relativeScroll * 5;
...
// Loop actualizado con hover detection
const animate = () => {
  ...
  // Si el modelo está en escena y activo
  if (model && model.visible && !isAnimating) {
    raycaster.setFromCamera(mouse, camera);
    const intersects = raycaster.intersectObject(model, true); // true = busca en children

    if (intersects.length > 0) {
      if (!isHovering) {
        isHovering = true;

        // Efecto de hover: pequeña rotación extra
        gsap.to(model.rotation, {
          x: model.rotation.x + 0.15,
          y: model.rotation.y + 0.2,
          duration: 0.5,
          ease: 'power2.out'
        });
      }
      ...
      if (isHovering) {
        ...
        // Regresa a su estado base suavemente
        gsap.to(model.rotation, {
          x: model.rotation.x - 0.15,
          y: model.rotation.y - 0.2,
          duration: 0.8,
...
animate(); // <-- ya lo tienes, así que simplemente asegúrate de tener esto en tu loop.
