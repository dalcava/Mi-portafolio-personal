<script>
    ...
    function preventScrollAboveStart() {
        if (!scrollContainer) return;

        const currentScroll = scrollContainer === window || scrollContainer === document.body || scrollContainer === document.documentElement
            ? window.scrollY
            : scrollContainer.scrollTop;

        if (currentScroll < SCROLL_START) {
            isProgrammaticScroll = true;

            if (scrollContainer === window || scrollContainer === document.body || scrollContainer === document.documentElement) {
                window.scrollTo({ top: SCROLL_START });
            } else {
                scrollContainer.scrollTop = SCROLL_START;
            }
    ...
    onMount(() => {
        ...
        if (scrollContainer) {
            scrollContainer.addEventListener('scroll', () => {
                ...
                preventScrollAboveStart(); // ⬅️ aquí lo invocás
            });
        }
