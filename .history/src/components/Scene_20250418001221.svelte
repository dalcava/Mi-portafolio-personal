...
const glassMaterial = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      vec3 color = mix(chroma, uTint, 0.15) + fresnel * vec3(1.2);

      // Extra core glow
      float centerGlow = pow(dot(normalize(vWorldNormal), normalize(vWorldPosition - cameraPosition)), 2.0);
      color += uTint * 0.2 * centerGlow;
