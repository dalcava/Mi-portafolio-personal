...
const glassMaterial = new THREE.ShaderMaterial({
  uniforms: {
    ...
    uChromaticAberration: { value: 0.02 },  // realistic prism split
    uFresnelPower: { value: 2.2 },
    uTint: { value: new THREE.Color('#24C887') } // gorgeous green!
...
void main() {
  ...
  vec3 chroma = vec3(refR.r, refG.g, refB.b) * 0.6;

  // Deep inner tint glow
  float coreGlow = pow(dot(normalize(vWorldNormal), normalize(vWorldPosition - cameraPosition)), 3.0);
  vec3 glowColor = uTint * coreGlow * 2.0;

  // Strong fresnel edges
  vec3 fresnelGlow = vec3(1.2, 0.8, 1.5) * fresnel * 1.2;

  // Combine layers
  vec3 color = mix(chroma, uTint, 0.35) + glowColor + fresnelGlow;

  // Optional: slight shadow tint for glass body
  vec3 bodyColor = vec3(0.02, 0.04, 0.03);
  color = mix(bodyColor, color, 0.93); // <- adds some contrast

  // Mid boost
  color = pow(color, vec3(1.0 / 1.15));

  float alpha = 0.94 + fresnel * 0.045;
