...
const glassMaterial = new THREE.ShaderMaterial({
  uniforms: {
    uTime: { value: 0 },
    uColor1: { value: new THREE.Color('#8ec5fc') },
    uColor2: { value: new THREE.Color('#e0c3fc') },
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      // Color shift based on view angle + time
      vec3 color = mix(uColor1, uColor2, 0.5 + 0.5 * sin(uTime + dot(vNormal, vViewPosition) * uIntensity));
      
      // Apply fresnel as glow effect
      vec3 finalColor = mix(color * 0.5, color, fresnel);
