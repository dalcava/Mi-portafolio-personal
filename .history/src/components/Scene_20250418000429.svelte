...
const sphere = new THREE.Mesh(geometry, divineGlassMaterial);
scene.add(sphere);
...
scene.environment = envMap;
    ...
    const divineGlassMaterial = new THREE.ShaderMaterial({
  uniforms: {
    uTime: { value: 0 },
...
function animate() {
  requestAnimationFrame(animate);
  divineGlassMaterial.uniforms.uTime.value += clock.getDelta();
  renderer.render(scene, camera);
}
animate();
