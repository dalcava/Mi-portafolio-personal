...
const glassMaterial = new THREE.ShaderMaterial({
  uniforms: {
    ...
    uChromaticAberration: { value: 1 },
    uFresnelPower: { value: 3.0 },
    uTint: { value: new THREE.Color('#a8e7ff') }
  },
  vertexShader: `
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      // Refraction with stronger RGB split
      ...
      // Slight desaturation to help tint pop
      float gray = dot(chroma, vec3(0.299, 0.587, 0.114));
      chroma = mix(vec3(gray), chroma, 0.7);

      // Main color blend
      vec3 color = mix(chroma, uTint, 0.25) + fresnel * vec3(1.4);

      // Internal soft glow
      float coreGlow = pow(dot(normalize(vWorldNormal), normalize(vWorldPosition - cameraPosition)), 3.0);
      color += uTint * 0.35 * coreGlow;

      // Sharper highlights
      color = pow(color, vec3(1.0 / 1.1)); // brighten mids

      // Stronger alpha
      float alpha = 0.97 + fresnel * 0.03;
