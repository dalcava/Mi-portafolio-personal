...
void main() {
  ...
  // Simulate dispersion glow inside
  float core = pow(dot(normalize(vWorldNormal), normalize(vWorldPosition - cameraPosition)), 2.0);
  vec3 coreGlow = vec3(0.7, 0.4, 1.0) * core * 1.5;

  // Rim glow — hot violet
  vec3 fresnelGlow = vec3(1.2, 0.7, 1.8) * fresnel;

  // Mix all layers
  vec3 color = mix(chroma * 0.4, uTint, 0.25) + coreGlow + fresnelGlow;
