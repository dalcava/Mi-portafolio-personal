...
<script>
  ...
  const tl = gsap.timeline();
      ...
      imageElements.forEach(img => {
        // Esperar completamente a que cargue
        img.addEventListener('load', () => {
          requestAnimationFrame(() => {
            const height = img.getBoundingClientRect().height;
            const rowHeight = 4; // igual a grid-auto-rows
            const span = Math.ceil(height / rowHeight);
            img.parentElement.style.gridRowEnd = `span ${span}`;
          });
        });

        // Si ya está cargada (por caché), dispara manualmente
        if (img.complete) {
          const height = img.getBoundingClientRect().height;
          const span = Math.ceil(height / 4);
          img.parentElement.style.gridRowEnd = `span ${span}`;
        }
      });
