...
<script>
  ...
  function handleScroll() {
    ...
    console.log('📍 scrollY:', scrollY); // Mostramos en consola
    ...
    // Mostrar dark
    if (scrollY >= 8200 && !showAbout) {
      showAbout = true;
    } else if (scrollY < 8200 && showAbout) {
      showAbout = false;
    }

    // Escalar video con el scroll
    const minScroll = 100;
    ...
    if (scrollY >= minScroll && scrollY <= maxScroll) {
      ...
      // Mostrar About
      if (scrollY >= 2000) {
        scrollContainer.classList.add('dark'); // 👈 usamos scrollContainer para agregar la clase
      } else {
        scrollContainer.classList.remove('dark'); // 👈 y para quitarla
      }
  }


  // Escuchamos el scroll del contenedor
...
.background.dark {
  background: linear-gradient(180deg, rgba(93, 0, 255, 1) 35%, rgba(20, 0, 51, 1) 95%) !important;
