...
<script>
  ...
  function smoothScroll(event) {
    ...
    const delta = event.deltaY * 0.95; // Ajusta la velocidad aquí
    const newScroll = scrollContainer.scrollTop + delta;

    gsap.to(scrollContainer, {
      scrollTop: newScroll,
      duration: 0.2,         // más largo = más suave
      ease: "power3.out"     // ease natural
    });
  }
