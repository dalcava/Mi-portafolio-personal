...
<script>
  ...
  function handleScroll() {
    ...
    if (scrollY >= 1500 && !isDark) {
      ...
      console.log('☀️ Desactivando modo dark');
    }
...
.background-overlay {
  ...
  width: 100%;
  height: 100%;
  background: linear-gradient(180deg, rgba(93, 0, 255, 0.8) 35%, rgba(20, 0, 51, 0.8) 95%);
  opacity: 0;
  pointer-events: none;
  transition: opacity 1s ease;
  z-index: 0; /* detrás de tus elementos, pero arriba del video */
}

:global(.background.dark) .background-overlay {
  opacity: 1;
}
