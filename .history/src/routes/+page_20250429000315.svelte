...
<script>
  ...
  function smoothScroll(event) {
    ...
    // Calculamos cuánto queremos movernos
    const delta = event.deltaY * 0.7; // ajusta la sensibilidad aquí

    // Sumamos el delta al objetivo
    targetScroll += delta;

    // Limitar para que no scrollee infinito negativo
    targetScroll = Math.max(0, Math.min(scrollContainer.scrollHeight - scrollContainer.clientHeight, targetScroll));

    // Si ya hay una animación corriendo, la actualizamos
    if (scrollTween) {
      scrollTween.kill();
    }

    // Hacemos la nueva animación hacia targetScroll
  ...
  scrollContainer.addEventListener('wheel', smoothScroll, { passive: false });
  ...
  return () => {
    ...
    scrollContainer.removeEventListener('wheel', smoothScroll);
  };
});
