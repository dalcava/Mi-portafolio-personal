...
<script>
  ...
  onMount(() => {
    ...
    const tl = gsap.timeline({ onComplete: () => loadingScreen.style.display = 'none' });
    tl.to(loadingScreen, { y: '100%', rotation: 0, ease: "power4.in", duration: 1.2 })
      .to(loadingScreen, { scaleX: 1, scaleY: 1.15, ease: "power2.out", duration: 0.2 }, "-=0.3")
      .to(loadingScreen, { scaleX: 1, scaleY: 1, rotation: 0, ease: "elastic.out(1, 0.35)", duration: 0.5 }, "-=0.3")
      .to(".background-img", { clipPath: "polygon(0 0, 100% 0, 100% 100%, 0 100%)", duration: 1.75, ease: "power2.out" }, "-=1.25");

    gsap.to('.Complementary-text', {
      opacity: 1, y: 0, duration: 1.2, delay: 1.2, ease: 'power3.out',
      onStart: () => document.querySelector('.Complementary-text')?.classList.remove('hidden')
    });
    ...
    function handleScroll() {
      ...
      // Escalar video
      const minScroll = 100, maxScroll = 1000;
    ...
    return () => backgroundEl.removeEventListener('scroll', handleScroll);
  });

  function restoreHomeAnimation() {
    const hero = document.querySelector('.hero');
    const pageTransition = document.getElementById('page-transition');
    if (hero) hero.style.opacity = '1', hero.classList.remove('fade-out');
...
<div class="background" bind:this={scrollContainer}>
  ...
  <div class="hero">
    <div class="background-elements">
      <video id="home-video" class="video-background" autoplay loop muted playsinline>
        <source src="{base}/Recursos/Fondos/Orbes.mp4" type="video/mp4" />
      </video>
      <CanvasParticles />
      <img src="{base}/Recursos/Fondos/Circle-text.gif" alt="Circle Text Animation" class="circle-text-gif" />
    </div>

    <div class="Complementary-text hidden">
      <h1> &nbsp;Hi! <br><br> &nbsp;I'm <br> <br> <span>David</span></h1>
  ...
  <section id="contact" class="footer-wrap">
    <Footer />
  </section>
