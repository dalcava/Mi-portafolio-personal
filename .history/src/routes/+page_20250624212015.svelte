...
<script>
  ...
  const isVerySmallScreen = writable(false);

  function updateScreenSize() {
    const height = window.innerHeight;
    isSmallScreen.set(height < 850);
    isVerySmallScreen.set(height < 700);
  }
 ...
 function handleScroll() {
  const rawScrollY = backgroundEl.scrollTop;
  const offset = $isSmallScreen ? -450 : 0;
  const scrollY = rawScrollY + offset;

  console.log('📍 scrollY ajustado:', scrollY);
  ...
  if ($isVerySmallScreen) {
    if (scrollY >= 5400 && !showAbout) {
      showAbout = true;
    } else if (scrollY < 5100 && showAbout) {
      showAbout = false;
    }
  } else if ($isSmallScreen) {
    if (scrollY >= 6500 && !showAbout) {
      showAbout = true;
    } else if (scrollY < 6500 && showAbout) {
      showAbout = false;
    }
  } else {
    if (scrollY >= 8500 && !showAbout) {
      showAbout = true;
    } else if (scrollY < 8000 && showAbout) {
      showAbout = false;
    }
  }

  // ✅ Modo dark ajustado con scrollY dinámico
if ($isVerySmallScreen) {
  if (scrollY > 5700 && isDark) {
    isDark = false;
    gsap.to(overlayEl, { opacity: 0, duration: 1 });
    console.log('🔅 Desactivando dark (muy pequeña pantalla, >5700)');
  } else if (scrollY >= 1000 && scrollY <= 5300 && !isDark) {
    isDark = true;
    gsap.to(overlayEl, { opacity: 1, duration: 1 });
    console.log('🌒 Activando dark (muy pequeña pantalla)');
  } else if (scrollY < 1000 && isDark) {
    isDark = false;
    gsap.to(overlayEl, { opacity: 0, duration: 1 });
    console.log('🔅 Desactivando dark (muy pequeña pantalla, <1000)');
  }
} else if ($isSmallScreen) {
  if (scrollY > 6800 && isDark) {
    isDark = false;
    gsap.to(overlayEl, { opacity: 0, duration: 1, ease: 'power3.out' });
    console.log('🔅 Desactivando dark (pequeña pantalla, scroll > 7500)');
  } else if (scrollY >= 1200 && scrollY <= 6500 && !isDark) {
    isDark = true;
    gsap.to(overlayEl, { opacity: 1, duration: 1, ease: 'power3.out' });
    console.log('🌒 Activando dark (pequeña pantalla)');
  } else if (scrollY < 1200 && isDark) {
    isDark = false;
    gsap.to(overlayEl, { opacity: 0, duration: 1, ease: 'power3.out' });
    console.log('🔅 Desactivando dark (pequeña pantalla, scroll < 1200)');
  }
} else {
  if (scrollY > 8400 && isDark) {
    isDark = false;
    gsap.to(overlayEl, { opacity: 0, duration: 1, ease: 'power3.out' });
    console.log('🔅 Desactivando dark (pantalla grande, scroll > 8400)');
  } else if (scrollY >= 1500 && scrollY <= 8100 && !isDark) {
    isDark = true;
    gsap.to(overlayEl, { opacity: 1, duration: 1, ease: 'power3.out' });
    console.log('🌒 Activando dark (pantalla grande)');
  } else if (scrollY < 1500 && isDark) {
    isDark = false;
    gsap.to(overlayEl, { opacity: 0, duration: 1, ease: 'power3.out' });
    console.log('🔅 Desactivando dark (pantalla grande, scroll < 1500)');
  }
}
...
<section id="about" class="about-wrap {showAbout ? 'show' : ''}">
...
<section id="contact" class="footer-wrap {showAbout ? 'show' : ''}">
...
.background-overlay {
  ...
  .footer-wrap {
    ...
    /* background-color: var(--Resalte-oscuro); */
    /* opacity: 0; */
    /* transition: opacity 0.6s ease-in-out; */
    min-height: 100vh;
  }
