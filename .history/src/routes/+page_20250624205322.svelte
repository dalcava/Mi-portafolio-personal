...
<script>
 ...
 function handleScroll() {
  ...
  const worksOffset = $isSmallScreen ? -500 : 0;
  const aboutOffset = $isSmallScreen ? -1000 : 0;

  const scrollYWorks = rawScrollY + worksOffset;
  const scrollYAbout = rawScrollY + aboutOffset;

  console.log('📍 scrollY raw:', rawScrollY);
  console.log('📍 scrollYWorks ajustado:', scrollYWorks);
  console.log('📍 scrollYAbout ajustado:', scrollYAbout);

  // Mostrar Works
  if (scrollYWorks >= 900 && !showWorks) {
    showWorks = true;
  } else if (scrollYWorks < 750 && showWorks) {
    showWorks = false;
  }

  // Mostrar About
  if (scrollYAbout >= 8500 && !showAbout) {
    showAbout = true;
  } else if (scrollYAbout < 8000 && showAbout) {
    showAbout = false;
  }

  // Modo dark
  if (rawScrollY > 8400 && isDark) {
    isDark = false;
    gsap.to(overlayEl, { opacity: 0, duration: 1, ease: 'power3.out' });
    console.log('Desactivando modo dark (por scroll muy alto)');
  } else if (rawScrollY >= 1500 && rawScrollY <= 8100 && !isDark) {
    isDark = true;
    gsap.to(overlayEl, { opacity: 1, duration: 1, ease: 'power3.out' });
    console.log('Activando modo dark');
  } else if (rawScrollY < 1500 && isDark) {
    isDark = false;
    gsap.to(overlayEl, { opacity: 0, duration: 1, ease: 'power3.out' });
  ...
  if (rawScrollY >= minScroll && rawScrollY <= maxScroll) {
    const progress = (rawScrollY - minScroll) / (maxScroll - minScroll);
  ...
  } else if (rawScrollY < minScroll) {
    video.style.width = `100%`;
    video.style.height = `100%`;
    video.style.borderRadius = `0px`;
  } else if (rawScrollY > maxScroll) {
