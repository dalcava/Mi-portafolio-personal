<script>
  ...
  // ✅ Agregamos valor inicial para evitar render con undefined
  let totalScrollHeight = SLIDE_COUNT * SCROLL_STEP + window.innerHeight;
  ...
  onMount(() => {
  // 🔁 Función que recalcula el alto total según el tamaño del viewport
  const updateScrollHeight = () => {
    totalScrollHeight = SLIDE_COUNT * window.innerHeight * 1.2 + window.innerHeight;
  };

  updateScrollHeight(); // Inicial
  ...
  // 🎯 Scroll y lógica del video
  function handleScroll() {
    const scrollY = backgroundEl?.scrollTop || 0;

    // Mostrar sección de Works
...
<div class="background" bind:this={scrollContainer}>
  ...
  <!-- Slider Sticky Section -->
  <div class="scroll-area">
    <div class="works-wrap" style="height: {totalScrollHeight}px;">
      <div class="works-container">
        <Works {scrollContainer} />
      </div>
    </div>
  </div>
